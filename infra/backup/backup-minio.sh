...
run_mirror() {
    ...
    # Il bucket potrebbe non esistere ancora (nessun media caricato): non è
    # un errore fatale, si riprova al ciclo successivo.
    mc mirror --overwrite --remove "source/${MINIO_BUCKET}" "${BACKUP_DIR}" \
        || echo "[backup-minio] Mirror non riuscito (bucket assente o non raggiungibile?), riprovo al prossimo ciclo." >&2
}
