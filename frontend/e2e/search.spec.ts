...
test.describe("Search", () => {
  ...
  test("shows a prompt before any filter is applied, not an empty table", async ({ page }) => {
    await page.goto("/search");
    await expect(page.getByText(/enter a phone number or apply a filter/i)).toBeVisible();
  });

  test("searching by a phone number that doesn't exist shows a real empty state", async ({ page }) => {
    await page.goto("/search");
    await page.getByPlaceholder("e.g. +39 345 678 9012").fill("+39 000 000 0000");
    ...
    await expect(page.getByText(/no records match these filters/i)).toBeVisible({ timeout: 10_000 });
  });

  test("filters are reflected in the URL and survive a reload", async ({ page }) => {
    await page.goto("/search");
    await page.getByPlaceholder("e.g. +39 345 678 9012").fill("+39 111 111 1111");
    await expect(page).toHaveURL(/phone=/);

    await page.reload();
    await expect(page.getByPlaceholder("e.g. +39 345 678 9012")).toHaveValue("+39 111 111 1111");
  });
  ...
  test("the Source Origin filter lists real seeded sources, not hardcoded placeholders", async ({
    ...
    await page.goto("/search");
    const sourceSelect = page.getByLabel("Source Origin");
